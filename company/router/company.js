...
router.get("/",[auth,checkUserRole("company")],async function(req,res){
    try {
...
router.get("/announcement",[auth,checkUserRole("company")], async function(req,res){
...
router.post('/announcement',upload.single('image'), [auth, checkUserRole('company')],async function(req,res){
...
router.get("/applications",[auth,checkUserRole("company")],async function(req,res){
    try {
        const company = await Company_model.findOne({ where: { id: req.user.id } });
        const applications = await Application_model.findAll({
			where: {
...
router.get("/internships",[auth,checkUserRole("company")],async function(req,res){
    try {
        const company = await Company_model.findOne({ where: { id: req.user.id } });
        const applications = await Application_model.findAll({
			where: {
...
router.get("/applications/:applicationId",[auth,checkUserRole("company")],async function(req,res){
    try {
        const company = await Company_model.findOne({ where: { id: req.user.id } });
...
router.post("/applications/:applicationId/fillApplicationForm",[auth,checkUserRole("company")],async function(req,res){
    try {
...
router.put("/applications/:applicationId",upload.single('upload-file'),[auth,checkUserRole("company")],async function(req,res){
    try {
		const applicationId = req.params.applicationId;
...
router.get("/applications/download/:applicationId/:fileType",[auth,checkUserRole("company")],async function(req,res){
    const applicationId = req.params.applicationId;
